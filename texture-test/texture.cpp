...
int main()
{
    ...
    data = stbi_load("HappyFish.jpg", &width, &height, &nrChannels, 0);
    if(data)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        glGenerateMipmap(GL_TEXTURE_2D);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        GLuint loc = glGetUniformLocation(shaderPro.ProgramID(), "transform");
        ...
        glBindTexture(GL_TEXTURE_2D, texture1);
        glUniform1i(glGetUniformLocation(shaderPro.ProgramID(), "ourTexture1"), 1);
